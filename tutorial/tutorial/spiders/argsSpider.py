...
class ArgsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        tag = getattr(self,'tag',None) #获取tag值，也就是命令行传过来的参数值
        if tag is not None: #判断是否存在tag参数，若存在，则重新构造url
            url = url + 'tag/' + tag #构造url，若tag=爱情，则url=“http://lab.scrapyd.cn/tag/爱情"
        yield scrapy.Request(url, self.parse) #发送请求爬取参数要求的内容
    ...
    def parse(self, response):
        ...
        # 我们需要继续提交给parse执行, scrapy 实现链接提交
